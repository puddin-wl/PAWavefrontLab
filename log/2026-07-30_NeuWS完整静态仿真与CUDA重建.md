...
当前共运行 20 项测试：19 项通过，1 项可选的 `1000×1000` CUDA 验收测试
按设计跳过。新增测试覆盖：
...
- 网络结果与指标全部有限；
- 物体归一化比例正确还原。
- 三维光声数据减 2048 后负值正确置零；
- 任意正层数都沿第 0 维最大投影，不要求 512 层；
- 真实多帧导入不破坏帧间相对强度。
...
- 完整静态仿真源码已提交为 `915621e`；本次三维光声预处理补充仍在工作区，
  尚未另行提交；
...
用户提供的旧脚本 `tif2048.py` 和 `layer.py` 明确了采集数据的两个约定：

1. 原始交流信号以像素值 2048 为零点；像素大于等于 2048 时减去 2048，
   否则置零；
2. 三维 TIFF 沿第 0 维取最大强度投影，得到二维图像。

本次进一步确认：继续按旧代码置零，始终沿第 0 维投影，但不再强制要求
第 0 维恰好为 512 层。

实现结果：

- 新增 `preprocessing/photoacoustic.py`，集中实现三维数组检查、浮点基线扣除、
  负值置零和第 0 维最大投影；
- 减法前转为 `float32`，避免 `uint16` 数据直接减 2048 时发生下溢；
- 步骤一的 `input_image` 现在可直接使用三维光声 TIFF；`input_mode="auto"`
  会识别常规二维/RGB 图片和光声体数据；
- 三维清晰参考图会额外保存
  `reference/photoacoustic_projection_raw.npy`，并在 `manifest.json` 中记录原始
  形状、数据类型、基线、投影维度和“不要求固定层数”的约定；
- 新增真实数据版入口
  `workflows/static_simulation/step3_import_photoacoustic_measurements.py`；它把
  一批三维 TIFF 转成连续编号的 `SLM_rawN.mat:imsdata`；
- 真实投影不自动裁剪或缩放，必须与 SLM/网络配置尺寸一致；
- MAT 文件保存原始投影强度，不对每帧单独归一化；训练和 PNG 预览使用一份
  数据集全局最大值，从而保留帧间相对强度。
...
步骤二生成 SLM 相位
    ...
    -> 每帧 max(raw - 2048, 0)
    -> 每帧沿 axis=0 最大投影
    -> SLM_rawN.mat:imsdata
    -> 步骤四 NeuWS 重建
```

新增自动测试覆盖了：3 层体数据也能正确处理、2048 以下置零、始终沿第 0
维投影、步骤一读取三维 TIFF、真实多帧导入保持帧间强度关系，以及生成的
MAT 文件可被现有 `BatchDataset` 直接加载。该补充是在已有提交
`915621e` 之后完成，当前尚未另行提交。
